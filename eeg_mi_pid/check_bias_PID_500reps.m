...
for freq=1:length(flows)
    ...
    for n=10:length(subs)
        ...
        for k=PCs
            ...
            disp("START PID SUBJECT: " + subs(n));
            exponent = 6;
            index = 1;
            while 2^exponent <= length(eeg)
                num_samples = 2^exponent;
                try
                    for iter=1:ITERATIONS
                        disp("Computing PID iteration: " + exponent + "_" + iter);
                        [red_all_ch,unq1_all_ch,unq2_all_ch,syn_all_ch] = PID_copnorm(eeg(:,1:num_samples),env(1:num_samples),pc(1:num_samples));
                        Rdn{k, index}{1, iter} = red_all_ch;
                        Unq1{k, index}{1, iter} = unq1_all_ch;
                        Unq2{k, index}{1, iter} = unq2_all_ch;
                        Syn{k, index}{1, iter} = syn_all_ch;
                    end
                    exponent = exponent + 2;
                    index = index + 1;
                catch
