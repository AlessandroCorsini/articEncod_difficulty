...
for freq=1:length(flows)
    for lag=lags_env
        ...
        if(COMPUTE_PID_PERFORMANCE)
            for n_performance=1:NUM_PERFORMANCE
                if(performances(n_performance))
                    try
                        Rdn_subs{n_performance} = load("results/PID/"+ fname_redundancy + fname_offset + fname_performance + string(n_performance) + "_" + fname_bandp).Rdn;
                        Unq1_subs{n_performance} = load("results/PID/" + fname_unq1 + fname_offset + fname_performance + string(n_performance) + "_" + fname_bandp).Unq1;
                        Unq2_subs{n_performance} = load("results/PID/" + fname_unq2 + fname_offset + fname_performance + string(n_performance) + "_" + fname_bandp).Unq2;
                        Syn_subs{n_performance} = load("results/PID/" + fname_synergy + fname_offset + fname_performance + string(n_performance) + "_" + fname_bandp).Syn;
                    catch
                        Rdn_subs{n_performance}={};
                        Unq1_subs{n_performance}={};
                        Unq2_subs{n_performance}={};
                        Syn_subs{n_performance}={};
                    end
                end
            end
        end
        ...
        for n=1:length(subs)
            ...
            if(COMPUTE_PID_DIFFICULTY)
                try
                    Rdn = {Rdn_subs{1}{SELECTED_SUB(n)}};
                    Unq1 = {Unq1_subs{1}{SELECTED_SUB(n)}};
                    Unq2 = {Unq2_subs{1}{SELECTED_SUB(n)}};
                    Syn = {Syn_subs{1}{SELECTED_SUB(n)}};
                    disp("try");
                catch
                    Rdn={};
                    Unq1={};
            ...
            if(COMPUTE_PID_PERFORMANCE)
                for n_performance=1:NUM_PERFORMANCE
                    if(performances(n_performance))
                        try
                            Rdn{n_performance} = Rdn_subs{n_performance}{n};
                            Unq1{n_performance} = Unq1_subs{n_performance}{n};
                            Unq2{n_performance} = Unq2_subs{n_performance}{n};
                            Syn{n_performance} = Syn_subs{n_performance}{n};
                        catch
                            Rdn{n_performance}={};
                            Unq1{n_performance}={};
                            Unq2{n_performance}={};
            ...
            if(COMPUTE_PID_PERFORMANCE)
                for n_performance=1:NUM_PERFORMANCE
                    if(performances(n_performance))
                        if(sum(dataSplit.goodPerformanceTrials{n_performance}) == sum(dataSplit.goodPerformanceTrials{n_performance} & validTrials') && sum(dataSplit.badPerformanceTrials{n_performance}) == sum(dataSplit.badPerformanceTrials{n_performance} & validTrials'))
                            disp("All correct");
                        end
                        meanRT_subjects_good{n_performance} = zeros(1,N_SUBJ);
                        meanRT_subjects_good{n_performance} = zeros(1,N_SUBJ);
                        accuracy_subjects_good{n_performance} = zeros(1,N_SUBJ);
                        accuracy_subjects_good{n_performance} = zeros(1,N_SUBJ);
                        lengths_subjects_good{n_performance} = zeros(1,N_SUBJ);
                        lengths_subjects_good{n_performance} = zeros(1,N_SUBJ);
                        
                        meanRT_subjects_good{n_performance}(n) = mean(behavior.RTs(dataSplit.goodPerformanceTrials{n_performance} & validTrials'));
                        meanRT_subjects_bad{n_performance}(n) = mean(behavior.RTs(dataSplit.badPerformanceTrials{n_performance} & validTrials'));
                        accuracy_subjects_good{n_performance}(n) = mean(behavior.answers(dataSplit.goodPerformanceTrials{n_performance} & validTrials'));
                        accuracy_subjects_bad{n_performance}(n) = mean(behavior.answers(dataSplit.badPerformanceTrials{n_performance} & validTrials'));
                        lengths_subjects_good{n_performance}(n) = sum(behaviorPhrases.lengths(behavior.phrases(dataSplit.goodPerformanceTrials{n_performance} & validTrials')));
                        lengths_subjects_bad{n_performance}(n) = sum(behaviorPhrases.lengths(behavior.phrases(dataSplit.badPerformanceTrials{n_performance} & validTrials')));
                    end
                end
            end
            ...
            lag_pc_sample = Fs*lag_pc;
            lag_env_sample = Fs*lag;
            for i=1:length(all_pcas_ordered)
                pcas_timeShifted{i}=all_pcas_ordered{i}(:,start_sample + lag_pc_sample:end - lag_pc_sample)';
                envelopes_timeShifted{i}=all_envelopes_ordered{i}(start_sample + lag_env_sample:end);
                lengths_timeShifted=[lengths_timeShifted length(envelopes_timeShifted{i})];
            end
            T=find(data_epochs.time{1}==start_time+lag_eeg);
            cfg=[];
            cfg.endsample=[];
            cfg.begsample=[];
            ...
            for i=1:length(data_epochs.trial)
                data_epochs_input.trial{1,i} = envelopes_timeShifted{i}';
                data_epochs_input.time{1,i} = data_epochs_input.time{1,i} - (lag_eeg - lag);
            end
            ...
            if fhigh ~=0
                ...
                cfg.padtype ='mirror';
                cfg.bpfilttype    = "but";
                data_epochs_env_filt = ft_preprocessing(cfg, data_epochs_input);
            end
            
            % PCs
            data_epochs_input.label=[];
            data_epochs_input.trial=[];
            data_epochs_input.time = data_epochs.time;
            % Iteration over all the 21 PCs
            for i=1:size(pcas_timeShifted{1},2)
                data_epochs_input.label{i,1}=char('PC'+string(i));
            end
            ...
            for i=1:length(data_epochs.trial)
                data_epochs_input.trial{1,i} = pcas_timeShifted{i}';
                data_epochs_input.time{1,i}  =data_epochs_input.time{1,i}-(lag_eeg - lag_pc);
            end
            if fhigh ~=0
                cgf=[];
                cfg.bpfilter = 'yes';
                ...
                cfg.padtype ='mirror';
                cfg.bpfilttype    = "but";
                data_epochs_pcs_filt = ft_preprocessing(cfg, data_epochs_input);
            end
            
            % Assigning the filtered signals back to the variables
            for i=1:length(data_epochs_env_filt.trial)
                envelopes_timeShifted{i}=data_epochs_env_filt.trial{i}(1,:)';
                pcas_timeShifted{i}=data_epochs_pcs_filt.trial{i}(1:end,:)';
            end
            ...
            for i=1:length(class1)
                ...
                for pid_iter=1:PID_ITERATIONS
                    ...
                    if(MATCH_LENGHT)
                        Rdn_subs{i}{SELECTED_SUB(n)}{pid_iter}=Rdn;
                        Unq1_subs{i}{SELECTED_SUB(n)}{pid_iter}=Unq1;
                        Unq2_subs{i}{SELECTED_SUB(n)}{pid_iter}=Unq2;
                        Syn_subs{i}{SELECTED_SUB(n)}{pid_iter}=Syn;
                    else
                        Rdn_subs{i}{SELECTED_SUB(n)}=Rdn;
                        Unq1_subs{i}{SELECTED_SUB(n)}=Unq1;
                        Unq2_subs{i}{SELECTED_SUB(n)}=Unq2;
                        Syn_subs{i}{SELECTED_SUB(n)}=Syn;
                    end
                end
            end
        end
